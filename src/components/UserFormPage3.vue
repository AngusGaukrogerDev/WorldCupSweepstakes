...
onMounted(() => {
        ...
        axios.get('https://sweep.brth.uk/data') //change to correct_url:3000/data
        .then((response) => {
...
function assignTeam(teamList){
    ...
    if(x<32)
    {
        axios.post('https://sweep.brth.uk/assigned', {"team": teamList[rand].team, "assignedUser": name.value})
...
function sendEmail() {
        ...
        axios.post('https://sweep.brth.uk/sendemail', {"name": formData.name, "email": formData.email, "team": formData.team });
