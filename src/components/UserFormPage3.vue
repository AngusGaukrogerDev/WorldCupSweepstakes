...
function assignTeam(teamList){
    ...
    if(x<32)
    {
        axios.post('https://localhost:3000//assigned', {"team": teamList[rand].team, "assignedUser": name.value})
...
function sendEmail() {
        ...
        axios.post('https://localhost:3000//sendemail', {"name": formData.name, "email": formData.email, "team": formData.team });
